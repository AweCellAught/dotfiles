...
curDirToWorkspacename = do
  ...
  when (isNothing name) $ do
    ...
    when (dir /= yHome) . WN.setCurrentWorkspaceName . last $ splitOneOf "/" dir
...
keyBindings XConfig {..} =
  ...
  combinedWorkspaceKeys ++ combinedScreenKeys ++ wsAndScreenFocuskeys
  where
    combinedWorkspaceKeys =
      workspaceKeys1 ++
      workspaceKeys2 ++
      workspaceKeys3 ++ workspaceKeys4 ++ workspaceKeys5 ++ layoutKeys
      where
        workspaceKeys1 =
          [ ( (m .|. modMask, key)
    ...
    layoutKeys
     ...
     =
      ...
      layoutAlgorithmKeys ++
      layoutChangeKeys ++
      layoutSpacingKeys ++ layoutRotationKeys ++ layoutReflectToggleEtc
      where
        windowKeysFocusMoveEtc =
...
createManageHook shifts floats =
  ...
  [ [isBrowserDialog --> forceCenterFloat] ++
    ...
    [isDialog --> doCenterFloat] ++ [pure True --> tileBelow]
  ]
...
braveClass, gtkFile :: IsString a => a
    ...
    ewmh
      zConfig
        { logHook =
            clickablePP
              xmobarPP
                { ppCurrent = xmobarColor colorGreen λ . wrap "<fn=0>" "</fn>" ----- Highlight the current workspace.
                , ppExtras =
                    [ gets $
                      Just .
                      show .
                ...
                , ppHidden =
                    xmobarColor colorAqua λ .
                    wrap "<fn=0>" "</fn>" . wrap "‹" "›"
                , ppLayout =
                    \t -> "(" ++ colorXmobar Magenta t ++ ")" -- Color the active layout name.
                , ppOutput = hPutStrLn xmproc
                , ppOrder = \(ws:l:t:ex) -> t : ws : l : ex -- The order of xmobars' outputs.
                , ppSep =
                    xmobarColor colorRed λ . -------- Seperators in xmobars.
                    wrap "<fn=2>" "</fn>" $
                    " :: "
                , ppSort =
                    (. filter \(W.Workspace tag _ _) -> isVisibleWS tag) <$>
                    getSortByIndex
                , ppTitle = xmobarColor colorAqua λ . wrap "<fn=0>" "</fn>"
                , ppUrgent = xmobarColor colorMagenta "" -- Highlight any urgent workspaces.
                , ppVisible =
                    xmobarColor colorTrueYellow λ .
                    wrap "<fn=0>" "</fn>" . wrap "«" "»"
                } >>=
            dynamicLogWithPP
        }
    ...
    zConfig =
      def
        ...
        , startupHook =
            do adjustEventInput
               spawn
                 "sleep 1 && trayer -l --edge bottom --align left --transparent true --alpha 0 --tint 000000 --SetPartialStrut true --widthtype request --monitor 0"
               spawnOnce discord
        ...
        , handleEventHook =
            ...
            debugKeyEvents <+> minimizeEventHook <+> floatConfReqHook yFloatCRMH
        } `additionalKeysP`
      [ ("M-s", promptedShift)
      , ("M-S-s", promptedGoto)
